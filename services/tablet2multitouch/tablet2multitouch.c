...
void send_multitouch_event(int uinput_fd, bool pressed, int tracking_id, __s32 x, __s32 y) {
    // input_mt_slot
    send_input_event(uinput_fd, EV_ABS, ABS_MT_SLOT, 0);
    if (pressed) {
        // input_mt_report_slot_state
        send_input_event(uinput_fd, EV_ABS, ABS_MT_TRACKING_ID, tracking_id);
...
void handle_event(int uinput_fd, struct input_event* ev) {
    ...
    switch (*type) {
        case EV_KEY:
            if (*code == BTN_LEFT) {
                pressed = !!*value;
                send_multitouch_event(uinput_fd, pressed, tracking_id, x, y);
                if (!pressed) {
                    tracking_id++;
